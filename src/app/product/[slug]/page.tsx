...
export default async function ProductPage({
  ...
  const specsObj: Record<string, string> = JSON.parse(product.specs);
  const specsEntries = Object.entries(specsObj);

  // Pick first 3 specs as quick highlights
  const highlights = specsEntries.slice(0, 3);

  const catUrlSlug =
    categories.find((c) => c.dbSlug === product.category.slug)?.urlSlug ?? "";
